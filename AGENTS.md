...
### ALSA / PocketBook Audio (Device)
- **PocketBook runs an `alsaloop` daemon at boot** (`alsaloop -Chw:Loopback,1,0 -P hp -t500000 --sync=none -U -v`) that exclusively grabs the hardware audio device (`hw:0,0`/`plughw:0,0`). Direct hardware opens return `-16` (EBUSY). User apps MUST write to the **Loopback device 0** (`plughw:1,0`); the Loopback card cross-connects device pairs (write to dev 0 playback → dev 1 capture → alsaloop → speaker). `dmix` also fails because the hardware is exclusively held. (source: `docs/devlog/20260615-issue39-on-device-audio-debugging-loopback-estrpipe-timer_log.md`)
- **ESTRPIPE is errno -86, NOT -77** — the Loopback device suspends the PCM stream after ~29s of playback; `snd_pcm_writei` returns `-86`. Recovery: `snd_pcm_resume()` first (clean), fall back to `snd_pcm_prepare()` if it returns `-ENOSYS` (-38), then retry the write. A wrong `-77` errno check caused audio to permanently die at ~29s. (source: same)
- **`stop_pipeline()` must call `sink.close()`** — otherwise every play→stop leaks one `snd_pcm_t` handle; the next `snd_pcm_open` returns EBUSY. `pause()` must NOT close the sink (keep it open for resume). (source: same)
- **Position display must be clock-based, not producer-PTS-based** — the decode producer runs ~12s ahead of playback (1 MiB ring buffer). `producer_position_ms` reflects the decoder's position, not the listener's. Use `position_ms + elapsed × speed` (wall clock). (source: same)
- **ALSA debug methodology**: `snd_strerror(ret)` confirms the error; `/proc/asound/card0/pcm0p/sub0/status` has `owner_pid` + `state` (OPEN/RUNNING); read `/proc/PID/cmdline` to identify the holder. `ffi.C.readlink` over `/proc/*/fd` may silently fail on PocketBook — prefer the status-file approach. (source: same)
- **Plugin Lua hot-reloads on navigation; C-level FDs do not.** Re-copying `.lua` files via USB and re-entering the plugin menu runs new code, but leaked `snd_pcm` handles belong to the running KOReader process. Only a full KOReader exit/restart (or device power cycle) releases them. Exiting to PocketBook home + relaunching is NOT a restart while USB-connected. (source: same)
...
| 2026-06-15 | issue | [issue39-on-device-audio-debugging-loopback-estrpipe-timer_log.md](docs/devlog/20260615-issue39-on-device-audio-debugging-loopback-estrpipe-timer_log.md) | On-device debugging gauntlet → **audible continuous playback achieved**: discovered PocketBook `alsaloop` daemon (`-Chw:Loopback,1,0 -P hp`) exclusively owns `hw:0,0` → retargeted ALSA output to Loopback `plughw:1,0`; fixed ESTRPIPE(-86) suspend recovery (was wrong -77) that killed audio at ~29s; fixed PCM handle leak (stop_pipeline never called sink.close); made position always clock-based (producer decodes ~12s ahead); fixed timer-display recurring tick; added `snd_pcm_resume`/`snd_strerror` FFI; 692 tests pass |
...
| 2026-06-14 | issue | [issue34-audio-playback-ffmpeg-alsa-audible-success_log.md](docs/devlog/20260614-issue34-audio-playback-ffmpeg-alsa-audible-success_log.md) | On-device verification: decode layout probe confirmed all FFmpeg 6.0 struct offsets (AVCodecParameters, AVCodecContext, AVFrame); play-test probe achieved AUDIBLE M4B playback via FFmpeg decode + swr_convert (FLTP→S16) + ALSA `snd_pcm_writei(plughw:0,0)`; key dead-ends: tts_sm is a virtual device that swallows audio, audio-engine open_alsa returns status code not pointer; 642 tests pass |
